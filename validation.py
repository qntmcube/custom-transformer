...
def greedy_decode(model: Transformer, src, enc_mask, tokenizer_src: Tokenizer, max_len, device):
    sos_token = tokenizer_src.token_to_id("[SOS]")
    eos_token = tokenizer_src.token_to_id("[EOS]")
    
    encoding = model.encode(src, enc_mask)
    
    decoder_output = torch.tensor([sos_token]).unsqueeze(0).type_as(src).to(device)
    
    while True:
        
        tgt_mask = causal_mask(decoder_output.size(1)).to(device).type_as(enc_mask)
        ...
        decoder_output = torch.cat([decoder_output, torch.tensor([best_token]).unsqueeze(0).type_as(src).to(device)], dim=1)
...
def run_validation(model: Transformer, val_dataloader, config, device, print_msg, tokenizer_src: Tokenizer, tokenizer_tgt: Tokenizer, num_val = 2):
    ...
    with torch.no_grad():
        ...
        for batch in val_dataloader:
            ...
            decoding = greedy_decode(model, encoder_input, encoder_mask, tokenizer_src, config["seq_len"], device)
